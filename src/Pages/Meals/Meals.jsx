...
const Meals = () => {
  ...
  useEffect(() => {
    ...
    const fetchFavorites = async () => {
      if (user && UsersAllDataFromDB?.email) {
        try {
          const response = await axios.get(
            `https://local-chef-bazar-backend-1.onrender.com/favorites?userEmail=${UsersAllDataFromDB.email}`,
          );
  ...
  const filteredAndSortedMeals = useMemo(() => {
    ...
    let filtered = meals.filter((meal) => {
      ...
      const matchesPrice =
        isNaN(mealPrice) ||
        (mealPrice >= Number(priceRange.min) &&
          mealPrice <= Number(priceRange.max));
      ...
      const matchesRating =
        selectedRating === "all" ||
        isNaN(mealRating) ||
        mealRating >= parseFloat(selectedRating || "0");

      // Location filter
      const matchesLocation =
        selectedLocation === "All Locations" ||
        !meal.location ||
        meal.location === selectedLocation;
  ...
  const refreshFavorites = async () => {
    if (user && UsersAllDataFromDB?.email) {
      try {
        console.log(
          "Manually refreshing favorites for user:",
          UsersAllDataFromDB.email,
        );
        const response = await axios.get(
          `https://local-chef-bazar-backend-1.onrender.com/favorites?userEmail=${UsersAllDataFromDB.email}`,
        );
  ...
  const handleFavorite = async (meal) => {
    ...
    try {
      if (isFavorited) {
        ...
        await axios.delete(
          `https://local-chef-bazar-backend-1.onrender.com/favorites/${meal._id}?userEmail=${UsersAllDataFromDB.email}`,
        );
        ...
        setFavorites((prev) => {
          const newFavorites = [
            ...prev,
            { ...payload, _id: response.data._id || "fav_" + Date.now() },
          ];
      ...
      toast.error(
        `Failed to update favorites: ${err.response?.data?.message || err.message}`,
      );
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-900">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <Helmet>
          ...
          <meta
            name="description"
            content="Browse and order delicious home-cooked meals from local chefs in your area."
          />
        </Helmet>

        {console.log(
          "Rendering Meals component, loading:",
          loading,
          "meals:",
          meals.length,
          "currentMeals:",
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="text-center sm:text-left">
              ...
              <p className="text-lg text-neutral-600 dark:text-neutral-400">
                Explore {filteredAndSortedMeals.length} delicious dishes from
        ...
        {currentMeals.length > 0 ? (
                  ...
                  meta={
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center space-x-1">
                         <span className="font-bold text-xl text-[#b41b1b]">Price : </span>
                        <span className="font-bold text-2xl text-[#b41b1b]">
                          ${meal.price}
                        </span>
                      </div>
                    </div>
          ...
          <div className="text-center py-20 bg-white dark:bg-neutral-800 rounded-2xl shadow-lg border border-neutral-200 dark:border-neutral-700">
            ...
            <p className="text-lg text-neutral-600 dark:text-neutral-400 mb-6 max-w-md mx-auto">
              It looks like we couldn't find any meals matching your current
              criteria. Try broadening your search or clearing the filters.
            </p>
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-12 p-4 bg-white dark:bg-neutral-800 rounded-2xl shadow-lg border border-neutral-200 dark:border-neutral-700">
            <div className="text-sm text-neutral-600 dark:text-neutral-400 text-center sm:text-left">
              ...
              of{" "}
              <span className="font-semibold text-red-600">{totalPages}</span>
            </div>
